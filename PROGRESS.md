...
- **`/api/chat` accepts an optional `document_ids` field** (spec extension over `docs/04`, which defined only `question`). Reason: document-scope selection (Feature 3) — the user can narrow which uploaded PDFs a query searches via checkboxes in the document list. Omit/empty = search all ready docs (backward compatible). Citations now also carry an `excerpt` of the source chunk (Feature 2: NotebookLM-style hoverable citations). Both are additive; the original `question`-only contract still works.
- **Assistant answers rendered as markdown** (Feature 1) on the frontend via `marked` + sanitized with `dompurify`. Reason: LLM output uses `**bold**`/lists; rendering raw `*` looked broken. Sanitization is mandatory before `v-html` since LLM output is untrusted (XSS).
